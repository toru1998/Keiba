...
def get_rawdata_results(race_id_list: list):
    ...
    """
    logger.info('preparing raw results table')
    race_results = {}
    ...
    with closing(sqlite3.connect(LocalPaths.DB_PATH)) as conn:
        ...
        for race_id in tqdm(race_id_list):
            try:
                ...
                if not result:
                    logger.warning(f'No data found for race_id {race_id}')
                    continue
                ...
                if horse_a_list is None:
                    logger.warning(f'No horse_id found for race_id {race_id}')
                    continue
                ...
                if jockey_a_list is None:
                    logger.warning(f'No jockey_id found for race_id {race_id}')
                    continue
                ...
                if trainer_a_list is None:
                    logger.warning(f'No trainer_id found for race_id {race_id}')
                    continue
                ...
                if owner_a_list is None:
                    logger.warning(f'No owner_id found for race_id {race_id}')
                    continue
                ...
                logger.error('error at {}'.format(race_id))
                logger.error(e)
...
def get_rawdata_info(race_id_list: list):
    ...
    """
    logger.info('preparing raw info table')
    race_infos = {}
    ...
    with closing(sqlite3.connect(LocalPaths.DB_PATH)) as conn:
        ...
        for race_id in tqdm(race_id_list):
            try:
                ...
                logger.error(f'error at race_id {race_id}')
                logger.error(e)
...
def get_rawdata_return(race_id_list: list):
    ...
    """
    logger.info('preparing raw return table')
    race_return = {}
...
def get_rawdata_horse_info(horse_id_list: list):
    ...
    """
    logger.info('preparing raw horse_info table')
    horse_info = {}
    ...
    with closing(sqlite3.connect(LocalPaths.DB_PATH)) as conn:
        ...
        for horse_id in tqdm(horse_id_list):
            try:
                ...
                result = cursor.fetchone()
                if not result:
                    logger.warning(f'No data found for horse_id {horse_id}')
                    continue
                ...
                logger.error(f'error at horse_id {horse_id}')
                logger.error(e)
...
def get_rawdata_horse_results(horse_id_list: list):
    ...
    """
    logger.info('preparing raw horse_results table')
    horse_results = {}
    ...
    with closing(sqlite3.connect(LocalPaths.DB_PATH)) as conn:
        ...
        for horse_id in tqdm(horse_id_list):
            try:
                ...
                if df.columns[0] == 0:
                    logger.warning(f'horse_results empty case1 {horse_id}')
                    continue
            ...
            # 競走データが無い場合（新馬）を飛ばす
                ...
                logger.warning(f'horse_results empty case2 {horse_id}')
                continue
            except Exception as e:
                logger.error(f'error at horse_id {horse_id}')
                logger.error(e)
...
def get_rawdata_peds(horse_id_list: list):
    ...
    """
    logger.info('preparing raw peds table')
    peds = {}
    ...
    with closing(sqlite3.connect(LocalPaths.DB_PATH)) as conn:
        ...
        for horse_id in tqdm(horse_id_list):
            try:
                ...
                if horse_a_list is None:
                    logger.warning(f'No pedigree table found for horse_id {horse_id}')
                    continue
...
def update_rawdata(filepath: str, new_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if os.path.isfile(filepath):
        ...
        # 結合データがない場合
        if new_df.empty:
            logger.info('preparing update raw data empty')
        else:
            # 元々のテーブルを読み込み
